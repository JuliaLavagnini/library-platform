...
describe('POST /api/books', () => {
  it('creates a book with every copy available', async () => {
    const res = await asLibrarian.post('/api/books').send(pragmatic).expect(201);
  ...
  it('returns every validation problem at once', async () => {
    const res = await asLibrarian.post('/api/books').send({ isbn: '123' }).expect(400);
  ...
  it('rejects a request with no body', async () => {
    await asLibrarian.post('/api/books').expect(400);
  });
});
...
describe('GET /api/books', () => {
  it('lists books sorted by title', async () => {
    ...
    const res = await asLibrarian.get('/api/books').expect(200);
  ...
  it('searches title and author, ignoring case', async () => {
    ...
    const byTitle = await asLibrarian.get('/api/books?search=PRAGMATIC').expect(200);
    const byAuthor = await asLibrarian.get('/api/books?search=fowler').expect(200);
  ...
  it('treats regex characters in a search as plain text', async () => {
    ...
    const res = await asLibrarian.get('/api/books').query({ search: '++ (' }).expect(200);
  ...
  it('filters to books with copies available', async () => {
    ...
    const res = await asLibrarian.get('/api/books?available=true').expect(200);

    expect(res.body.map((b: { title: string }) => b.title)).toEqual(['Refactoring']);
  });

  it('rejects an invalid filter', async () => {
    await asLibrarian.get('/api/books?available=maybe').expect(400);
  });
});
...
describe('GET /api/books/:id', () => {
  it('returns the book', async () => {
    const { id } = await createBook();
    const res = await asLibrarian.get(`/api/books/${id}`).expect(200);
  ...
  it.each([
    ...
    const res = await asLibrarian.get(`/api/books/${id}`).expect(404);
...
describe('PATCH /api/books/:id', () => {
  ...
  it('keeps copies on loan when total copies change', async () => {
    ...
    const res = await asLibrarian.patch(`/api/books/${id}`).send({ totalCopies: 5 }).expect(200);
  ...
  it('refuses to reduce total copies below the number on loan', async () => {
    ...
    const res = await asLibrarian.patch(`/api/books/${id}`).send({ totalCopies: 1 }).expect(409);
  ...
  it('rejects an empty update', async () => {
    const { id } = await createBook();
    await asLibrarian.patch(`/api/books/${id}`).send({}).expect(400);
  });

  it('returns 404 for an unknown book', async () => {
    await asLibrarian.patch(`/api/books/${unknownId}`).send({ title: 'x' }).expect(404);
  });
});
...
describe('DELETE /api/books/:id', () => {
  it('deletes a book with no copies on loan', async () => {
    ...
    await asLibrarian.delete(`/api/books/${id}`).expect(204);
    await asLibrarian.get(`/api/books/${id}`).expect(404);
  });
  ...
  it('refuses to delete a book with copies on loan', async () => {
    ...
    await asLibrarian.delete(`/api/books/${id}`).expect(409);
    await asLibrarian.get(`/api/books/${id}`).expect(200);
  });
});
...
describe('POST /api/books/:id/borrow and /return', () => {
  it('takes a copy off the shelf and puts it back', async () => {
    ...
    const borrowed = await asService.post(`/api/books/${id}/borrow`).expect(200);
    expect(borrowed.body.availableCopies).toBe(1);

    const returned = await asService.post(`/api/books/${id}/return`).expect(200);
  ...
  it('refuses to borrow when no copies are left', async () => {
    ...
    const res = await asService.post(`/api/books/${id}/borrow`).expect(409);
  ...
  it('refuses to return when every copy is already on the shelf', async () => {
    const { id } = await createBook();
    await asService.post(`/api/books/${id}/return`).expect(409);
  });

  it.each(['borrow', 'return'])('returns 404 when trying to %s an unknown book', async (action) => {
    await asService.post(`/api/books/${unknownId}/${action}`).expect(404);
  });
});

describe('unknown routes', () => {
  it('returns a JSON 404', async () => {
    const res = await asLibrarian.get('/api/nothing-here').expect(404);
